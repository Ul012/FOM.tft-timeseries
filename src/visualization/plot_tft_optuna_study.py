# src/visualization/plot_optuna_study.py
"""
...
def plot_study(study_name: str, plots: list[str]):
    ...
    print(f"[plot_optuna_study] Study: {study_name}")
    print(f"[plot_optuna_study] Trials: {len(study.trials)} (davon {len(completed)} abgeschlossen)")
    print(f"[plot_optuna_study] Beste val_mae: {study.best_value:.4f}")
    print()
    ...
    for plot_name in plots:
        ...
        try:
            ...
            print(f"[plot_optuna_study] ✓ {html_path}")

            # PNG nur wenn kaleido verfügbar
            try:
                png_path = plot_dir / f"{filename}.png"
                fig.write_image(str(png_path))
                print(f"[plot_optuna_study] ✓ {png_path}")
            except Exception:
                print(f"[plot_optuna_study] ⚠ PNG-Export übersprungen (kaleido nicht installiert)")

        except Exception as e:
            print(f"[plot_optuna_study] ✗ Fehler bei {plot_name}: {e}")

    print()
    print(f"[plot_optuna_study] Plots gespeichert in: {plot_dir}")
...
def main():
    parser = argparse.ArgumentParser(description="Visualisiere Optuna Study")
    parser.add_argument(
        "--study-name",
        type=str,
        required=True,
        help="Name der Optuna Study (z.B. tft_day)",
    )
    parser.add_argument(
        "--plots",
        type=str,
