...
def _plot_series_forecast(
    ...
    ax.set_xlabel("Zeit", color=COLORS['text'])
    ax.set_ylabel("Verkäufe", color=COLORS['text'])
    ...
    # Kompakter Titel
    ax.set_title(f"TFT - Vorhersage vs. Ist ({split}) | {short_run}", color=COLORS['text'])
    ...
    # Details als Untertitel unter der Grafik
    legend = ax.legend()
    ...
    fig.text(0.5, 0.04, " | ".join(detail_parts),
        ha = 'center', fontsize = 9, style = 'italic', color = COLORS['text'],  # ← nur color hinzufügen
...
def main() -> None:
    ...
    output_path = plots_root / f"{run_id}_{split}_forecast_series_dark.png"
    ...
    # CSV speichern
    csv_path = plots_root / f"{run_id}_{split}_forecast_details_dark.csv"
...
# Aufruf:
#   $env:DATASET_CONFIG="configs/datasets/booksales.yaml"; python -m src.visualization.plot_tft_forecast_series_dark --run-id run_20260111_113916_booksales_optuna_tft_newyear_best --split test --history-length 120
#   $env:DATASET_CONFIG="configs/datasets/walmart.yaml"; python -m src.visualization.plot_tft_forecast_series_dark --run-id run_20260111_113926_walmart_optuna_walmart_full_best --split test --history-length 120