# src/evaluation/analyze_optuna_prophet_trials.py
"""
Analysiert alle Prophet Optuna Trials und erstellt detaillierte Berichte.

Aufruf:
    Booksales:
    $env:DATASET_CONFIG="configs/datasets/booksales.yaml"
    python -m src.evaluation.analyze_optuna_prophet_trials --study-name prophet_booksales

    Walmart:
    $env:DATASET_CONFIG="configs/datasets/walmart.yaml"
    python -m src.evaluation.analyze_optuna_prophet_trials --study-name prophet_walmart --top-n 10
"""
...
def analyze_all_trials(study_name: str, top_n: int = 10):
    ...
    print("=" * 80)
    print(f"PROPHET OPTUNA TRIAL-ANALYSE: {study_name}")
    print("=" * 80)
    print()
    ...
    for col in param_cols:
        ...
        if df_complete[col].dtype in ["float64", "int64"]:
            print(f"\n{param_name}:")
            print(f"  Min:    {df_complete[col].min():.6f}")
            print(f"  Max:    {df_complete[col].max():.6f}")
            print(f"  Mean:   {df_complete[col].mean():.6f}")
            print(f"  Median: {df_complete[col].median():.6f}")
        else:
            print(f"\n{param_name}:")
    ...
    for param, corr in correlations:
        ...
        print(f"  {param:<30} {direction} {corr:>7.4f}")
...
def main():
    parser = argparse.ArgumentParser(description="Analysiere Prophet Optuna Trials")
    parser.add_argument(
