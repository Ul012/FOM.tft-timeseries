# src/evaluation/aggregate_arima_eval.py
# Aggregiert alle ARIMA eval_val.json + eval_test.json zu einer Übersichtstabelle.
#
# ARIMA-Struktur: eval_val.json und eval_test.json sind getrennt (nicht kombiniert wie TFT)
#
# Nutzung:
...
def _load_single_run(run_dir: Path) -> Dict[str, Any] | None:
    """
    Lädt eval_val.json + eval_test.json für einen ARIMA-Run und kombiniert sie.
    ...
    try:
        ...
        print(f"[aggregate_arima_eval] Warnung: Fehler beim Laden von {run_dir.name}: {e}")
        return None
...
def aggregate_evaluations(runs_root: Path) -> pd.DataFrame:
    """
    Sucht nach ARIMA-Runs (Ordner mit eval_val.json + eval_test.json)
    ...
    if not rows:
        raise FileNotFoundError(
            f"Keine ARIMA-Evaluierungen gefunden in {runs_root}. "
            f"Bitte zuerst evaluate_arima.py ausführen."
        )
...
def main() -> None:
    runs_root = BASE_DIR / "results" / "arima" / "runs"

    if not runs_root.exists():
        raise FileNotFoundError(f"ARIMA runs Verzeichnis nicht gefunden: {runs_root}")

    df = aggregate_evaluations(runs_root)

    # Output ins arima-Verzeichnis (analog zu TFT)
    output_dir = BASE_DIR / "results" / "arima"
    ...
    print("[aggregate_arima_eval] Aggregation abgeschlossen.")
...
# Aufruf (nachdem ARIMA-Modelle evaluiert wurden):
#   python -m src.evaluation.aggregate_arima_eval
#
# Erzeugt: results/arima/eval_overview.{csv,json}